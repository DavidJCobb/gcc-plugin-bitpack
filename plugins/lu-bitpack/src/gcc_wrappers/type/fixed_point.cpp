...
namespace gcc_wrappers::type {
   WRAPPED_TREE_NODE_BOILERPLATE(fixed_point)
   ...
   fixed_point fixed_point::make_signed() const {
      return fixed_point::from_untyped(signed_type_for(this->_node)); // tree.h
   }
   fixed_point fixed_point::make_unsigned() const {
      return fixed_point::from_untyped(unsigned_type_for(this->_node)); // tree.h
   }
}