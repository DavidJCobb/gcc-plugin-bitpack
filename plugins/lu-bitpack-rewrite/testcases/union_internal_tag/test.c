...
union LU_BP_UNION_INTERNAL_TAG(tag) TestUnion {
...
static union TestUnion sTestUnion[2];
...
void print_test_data() {
   for(int i = 0; i < 2; ++i) {
      printf("TestUnion[%u] == {\n", i);
      printf("   .*.header = %u,\n", sTestUnion[i].a.header);
      printf("   .*.tag    = %u,\n", sTestUnion[i].a.tag);
      if (sTestUnion[i].a.tag == 0) {
         printf("   .a = {\n");
         printf("      .data = %u\n", sTestUnion[i].a.data);
         printf("   },\n");
      } else if (sTestUnion[i].a.tag == 1) {
         printf("   .b = {\n");
         printf("      .data = %u\n", sTestUnion[i].b.data);
         printf("   },\n");
      }
...
int main() {
   ...
   //
   sTestUnion[0].a.header = 11;
   sTestUnion[0].a.tag    = 0;
   sTestUnion[0].a.data   = 5555;
   sTestUnion[1].a.header = 22;
   sTestUnion[1].a.tag    = 1;
   sTestUnion[1].b.data   = 12345678;
   ...
   memset(&sTestUnion, 0xCC, sizeof(sTestUnion));
