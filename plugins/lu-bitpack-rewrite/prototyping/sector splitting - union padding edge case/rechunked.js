...
rechunked.chunks.padding = class padding extends rechunked.chunks.base {
   constructor() {
      super();
      this.bitcount = 0;
   }
...
rechunked.item = class item {
   ...
   from_serialization_item(item) {
      for(let segm of item.segments) {
         ...
         if (segm.padding !== null) {
            let back = new rechunked.chunks.padding();
            back.bitcount = segm.padding;
            this.chunks.push(back);
            continue;
         }
