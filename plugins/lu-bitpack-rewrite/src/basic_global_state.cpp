...
static location_t _location_of(tree node) {
   if (DECL_P(node)) {
      return gw::decl::base::from_untyped(node).source_location();
   }
   if (EXPR_P(node)) {
      return gw::expr::base::from_untyped(node).source_location();
   }
...
void basic_global_state::pull_builtin_functions() {
   {
      ...
      if (decl.empty()) {
         decl = gcc_wrappers::decl::function::from_untyped(
            lookup_name(get_identifier("memcpy"))
         );
         if (decl.empty()) {
            decl = gcc_wrappers::decl::function::from_untyped(
               lookup_name(get_identifier("__builtin_memcpy"))
            );
         }
      }
   }
   {
      ...
      if (decl.empty()) {
         ...
         if (decl.empty()) {
            decl = gcc_wrappers::decl::function::from_untyped(
               lookup_name(get_identifier("__builtin_memset"))
            );
         }
      }
   }
