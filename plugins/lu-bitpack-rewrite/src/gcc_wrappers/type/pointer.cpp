...
namespace gcc_wrappers::type {
   ...
   bool pointer::compatible_address_space_with(pointer other) const {
      ...
      auto as_a = TYPE_ADDR_SPACE(target_a.as_untyped());
      auto as_b = TYPE_ADDR_SPACE(target_b.as_untyped());
      if (as_a == as_b)
         return true;
   ...
   bool pointer::same_target_and_address_space_as(pointer other) const {
      ...
      auto strip_a  = base::from_untyped(strip_array_types(main_a.as_untyped()));
      auto strip_b  = base::from_untyped(strip_array_types(main_b.as_untyped()));
      //
