...
* Implement the union-related attributes, so that `serialization_item`s expanded from a struct can reflect union tag-related conditions.
  * `lu_bitpack_union_external_tag`
    * Conditions enforceable from the attribute handler:
      * Only valid when applied to a FIELD_DECL whose type is a UNION_TYPE (hereafter: "the annotated member").
      * Invalid if any member of the annotated member's `UNION_TYPE` has neither of `lu_bitpack_tagged_id` or `lu_bitpack_omit`. (The `UNION_TYPE` has to be complete for us to check this, but if it isn't by the time we see the annotated member, then that's a C/C++ error. We don't need to report our own error, but we should still mark the attribute as failed so that codegen will fail.)
      * Invalid if any member of the annotated member's `UNION_TYPE` has both of `lu_bitpack_tagged_id` and `lu_bitpack_omit`, but doesn't have `lu_bitpack_default_value`. (The `UNION_TYPE` has to be complete for us to check this, but if it isn't by the time we see the annotated member, then that's a C/C++ error. We don't need to report our own error, but we should still mark the attribute as failed so that codegen will fail.)
      * Invalid if any two members of the annotated member's `UNION_TYPE` specify the same value for `lu_bitpack_tagged_id`. (The `UNION_TYPE` has to be complete for us to check this, but if it isn't by the time we see the annotated member, then that's a C/C++ error. We don't need to report our own error, but we should still mark the attribute as failed so that codegen will fail.)
    * Conditions enforceable when the annotated member's containing type finishes parsing:
      * Invalid if the containing type is not a `RECORD_TYPE` (including being invalid if it's a `UNION_TYPE`).
      * Invalid if the referenced tag member doesn't exist on the immediate containing struct type.[^nested-unions-are-hard]
      * Invalid if the referenced tag member comes after the annotated member.
  * `lu_bitpack_union_internal_tag`
    * Requirements
      * **[Applied to the right thing.]** Only valid when applied to a UNION_TYPE, to a TYPE_DECL of a UNION_TYPE, or to a FIELD_DECL whose type is a UNION_TYPE.
        * Verify in the attribute handler.
      * **[Union of struct-type members.]** Only valid if all FIELD_DECLs of the UNION_TYPE are of struct types (RECORD_TYPE).
      * **[Common headers.]** Only valid if the first *n* members of those RECORD_TYPEs are identical for some value of *n* > 0, and if the specified identifier (the attribute argument) is the name of one of those members.
      * **[Uniquely ID'd members.]** Only valid if all union members also have `lu_bitpack_tagged_id` or `lu_bitpack_omit`, and if the IDs across all members are unique.
      * **[Omitted and ID'd must have a default.]** Only valid if any union members that have both `lu_bitpack_tagged_id` and `lu_bitpack_omit` also have `lu_bitpack_default_value`.
    * When requirements should be checked, based on what the attribute was applied to:
      * `FIELD_DECL`
        * All requirements can be checked in the attribute handler. (The `UNION_TYPE` has to be complete for us to check it for compliance, but if it isn't by the time we see the annotated member, then that's a C/C++ error. We don't need to report our own error, but we should still mark the attribute as failed so that codegen will fail.)
      * `UNION_TYPE`
        * `PLUGIN_FINISH_TYPE`
      * `TYPE_DECL` of a `UNION_TYPE`
        * `PLUGIN_FINISH_TYPE`. When using `typedef` to create a synonym of a union type, the type doesn't have to be complete yet. Ergo in `PLUGIN_FINISH_TYPE`, when a `UNION_TYPE` is complete, we must validate the transitive typedef chain, being careful to emit errors on the entity that actually has the attribute and not just wherever.
* Implement expanding `serialization_item`s that refer to unions.[^expanding-unions]
  * The expanded serialization items should have a condition related to the union tag.
...
[^nested-unions-are-hard]:

    Nested unions are tricky. Consider the following case:

    ```c
    struct Foo {
    int tag;
    struct Bar {
    __attribute__((lu_bitpack_union_external_tag("tag"))) union {
    // ...
    } baz;
    } bar;
    };
    ...
    It might seem, intuitively, like that should work, but it actually shouldn't: since `Bar` is a named type, it can be instantiated separately as `Foo::Bar orphan`, such that `orphan.baz` would have no containing `Foo` instance with a `tag` for us to apply.

    Technically, even this is unsafe:
    ...
    In C, it might not be possible to break that, but in C++, one could spawn a `decltype(myFoo::bar) orphan`.

    Ergo, an externally-tagged union should only be allowed to refer to fields of the immediate enclosing struct type. This is something we can validate at the time that we receive `PLUGIN_FINISH_TYPE` for that type.
...
[^expanding-unions]:
    ...
    * `if (sFoo.a.tag == 0) PADDING, 40 bits`
