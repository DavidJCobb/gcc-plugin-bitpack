...
* Attribute handlers: set up some simple testcases to verify that it's correct to create and return a new attribute node (as we do in `attribute_handlers::generic_type_or_decl`) in order to defer the addition of the potential attribute to some later node. (If it's not valid, then what the hell *do* we return?)
